...
module.exports = {
  ...
  submitCallRequest: asyncHandler(async (req, res, next) => {
    const { name, phone, email, interestedTopic } = req.body;
    if (!name || !phone || !email) {
      return errorResponse(res, 400, messageHelper.BAD_REQUEST);
    }
    const callRequestData = await saveCallRequestToDB({
      name,
      phone,
      email,
      interestedTopic,
    ...
    return successResponse(
      res,
      200,
      messageHelper.CALL_REQUEST_SUBMITTED,
      callRequestData
    );
  }),
  ...
  getAllCallRequests: asyncHandler(async (req, res, next) => {
    ...
    return successResponse(
      res,
      200,
      messageHelper.CALL_REQUEST_DATA_FETCHED,
      callRequestData
    );
  }),
  ...
  editCallRequestStatus: asyncHandler(async (req, res, next) => {
    ...
    return successResponse(
      res,
      200,
      messageHelper.CALL_REQUEST_STATUS_UPDATED,
      callRequestData
    );
  }),
