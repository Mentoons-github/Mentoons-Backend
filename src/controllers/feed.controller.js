...
const getUserFeed = async (req, res) => {
  try {
    ...
    if (req.user && req.user.dbUser) {
      ...
      const blockedUsers = userFeed.blockedUsers || [];
      const hiddenPosts = userFeed.hiddenPosts || [];
      ...
      const posts = await Post.aggregate([
        ...
        },
        {
          $addFields: {
            isFollowed: {
              $cond: [{ $in: ["$user", followedUserIds] }, 1, 0],
            },
          },
        },
        {
      ...
      return res.status(200).json({
        ...
        pagination: {
          page: parsedPage,
          limit: parsedLimit,
          hasMore: populatedPosts.length === parsedLimit,
        },
      });
    }
    ...
    const posts = await Post.paginate({ visibility: "public" }, options);
